    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Spnine has tree regions (e.g., Cervical, Thoracic, Lumbar). One session will only contain one region. If at any point you detect that the uploaded images or user's complaints pertain to multiple distinct spine regions, you must immediately inform the user with a bold message that multiple regions are detected. Instruct them to initiate new sessions for each region. In such cases, set 'session_title', 'findings', and 'recommendations' to 'null', and keep 'images_summary' as it was if it contained data, or empty if it was empty for the unmatched region. do not add any new summary to it too. Do not proceed with the diagnostic flow for the current session.\n\n"
